...
public class Heuristic2
{
    ...
    /**
     * Create an instance of Heuristic2 and run a tour. Each time you call this
     ...
     *            the starting column
     */
    public Heuristic2(final int row, final int column)
    {
        currentRow = row ;
        currentCol = column ;
     ...
     * than one move is possible, then choose the one from which the square with
     * the lowest accessibility number may be reached.
     * 
     * @return true if a move was made and false otherwise.
     */
    private boolean nextMove()
    {
        int[] move = { 0, 0, 0, 0, 0, 0, 0, 0 } ;
        int[] nextMoveAccess = { 0, 0, 0, 0, 0, 0, 0, 0 } ;
        int position = 0 ;
        int access = 9 ;
        for (int i = 0; i < MOVES; ++i)
        {
            ...
            if (legalMove(r, c))
            {
                if (accessibility[r][c] < access)
                {
                    ...
                    move[0] = i ;
                    position = 1 ;
                    nextMoveAccess[0] = testMove(r, c, currentRow, currentCol) ;
                }
                else if (accessibility[r][c] == access)
                {
                    move[position] = i ;
                    nextMoveAccess[position] = testMove(r, c, currentRow,
                            currentCol) ;
                    ++position ;
                }
            }
        ...
        {
            int myMove = move[0] ;
            access = nextMoveAccess[0] ;
            for (int i = 1; i < position; ++i)
            {
                if (nextMoveAccess[i] < access)
                {
                    myMove = move[i] ;
                    access = nextMoveAccess[i] ;
                }
            }
            reduceAccess() ;
            currentRow += HORIZONTAL[myMove] ;
            currentCol += VERTICAL[myMove] ;
            return true ;
        }
    }
    ...
    /**
     * Return the lowest accessbility number for a square that can be reached
     * from (row, column) provided the square is not at (notRow, notCol).
     * 
     * @param row -
     *            the row of the square to move from
     * @param col -
     *            the column of the square to move from
     * @param notRow -
     *            the row of the current square
     * @param notCol -
     *            the column of the current square
     * @return the lowest accessbility number reachable from here
     */
    private int testMove(final int row, final int col, final int notRow,
            final int notCol)
    {
        int access = 9 ;
        for (int i = 0; i < MOVES; ++i)
        {
            int r = row + HORIZONTAL[i] ;
            int c = col + VERTICAL[i] ;
            if (legalMove(r, c) && !(r == notRow && c == notCol)
                    && accessibility[r][c] < access)
            {
                access = accessibility[r][c] ;
            }
        }
     ...
     * debugging the accessibility matrix if necessary, although that turned out
     * to be fine
     * 
     * @param a -
